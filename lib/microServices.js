...
function addClient(url, application, startParams) {
  ...
  debug('qdding MicroService Client connection: url: %s; application: %s', url, application);
...
module.exports = function(services_config) {
  ...
  services_config.routes.forEach(function(service) {
    ...
    debug('route config: %s', JSON.stringify(service));
    ...
    if (path) {
      ...
      debug('rest route: %s', JSON.stringify(route));
  ...
  debug('u_services by application: %s', JSON.stringify(q.u_services.byApplication));
