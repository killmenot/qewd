...
var koaRouter = Router().loadMethods(); // eslint-disable-line new-cap
var koaServe = require('koa-static');
var koaBodyParser = require('koa-bodyparser');
var debug = require('debug')('qewd:master-koa');
...
function qxHandleMessage(ctx) {
  debug('params: %s', JSON.stringify(ctx.params));
...
async function responseTime (ctx, next) {
  ...
  debug('X-ResponseTime: %s', elapsed);
...
function configure(config, routes, qOper8, qExt) {
  ...
  if (config.bodyParser) {
    debug('using custom body parser module');
    bodyParser = config.bodyParser;
  }
  else {
    debug('using default body-parser module');
  ...
  debug('webServerRootPath: %s', config.webServerRootPath);
  ...
  koaRouter.addRoute('POST /ajax', async (ctx, next) => {
    debug('POST /ajax: %s', JSON.stringify(ctx.request.body));
    ctx.request.headers.qewd = 'ajax';
    await qxHandleMessage(ctx);
    await next();
  });
  ...
  if (routes) {
    ...
    routes.forEach(function(route) {
      ...
      if (route.beforeRouter && Array.isArray(route.beforeRouter)) {
        debug('configuring route.beforeRouter: %s', JSON.stringify(route.beforeRouter));
        args = args.concat(route.beforeRouter);
      }
      ...
      if (route.afterRouter) {
        handler = async (ctx, next) => {
          ...
          debug('state: %s', JSON.stringify(ctx.state));
      ...
      if (route.afterRouter && Array.isArray(route.afterRouter)) {
        debug('configuring route.afterRouter: %s', JSON.stringify(route.afterRouter));
        args = args.concat(route.afterRouter);
      }

      debug('adding koa routes for path %s, args: %s', path, JSON.stringify(args));
  ...
  app.use(ctx => {
    debug('end of middleware chaing');
    debug('ctx.state: %s', JSON.stringify(ctx.state));
