...
function storeIncomingMessage(message) {
  ...
  debug('storing incomming message: %s', JSON.stringify(message));
  debug('timestamp index: %s', ix);
...
function removePendingIndex(token, ix) {
  ...
  debug('removing pending index: %s', glo);
  setTimeout(function() {
...
function cleardownQueueBackup() {
  ...
  messages.forEachChild(function(dbIndex, messageObj) {
    ...
    if (timestamp < cutOff) {
      ...
      // cutoff reached, so stop checking the queue backup document
      debug('stop checking the queue backup document');
      return true;
    }
  });
