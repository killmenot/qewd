...
var bodyParser;
var app = express();
var debug = require('debug')('qewd:master-express');
...
function configure(config, routes, qOper8, qExt) {
  ...
  app.post('/ajax', function(req, res) {
    debug('/ajax: %s', JSON.stringify(req.body));
  ...
  if (routes) {
    ...
    routes.forEach(function(route) {
      ...
      // allow user to define custom middleware for each route
      app.use.apply(app, args);

      debug('route ' + route.path + ' will be handled by qx.router');
    });
  }
