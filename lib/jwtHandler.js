...
var key;
var iv;
var debug = require('debug')('qewd:jwtHandler');
...
function decrypt(text, secret) {
  ...
  if (!key) {
    ...
    debug('decrypt: key = %s', results[0]);
    debug('decrypt: iv = %s', results[1]);
...
function encrypt(text, secret) {
  if (!key) {
    var results = compute(algorithm, secret);
    debug('encrypt: key = %s', results[0]);
    debug('encrypt: iv = %s', results[1]);
    key = Buffer.from(results[0], 'hex');
    iv = Buffer.from(results[1], 'hex');
  }
...
function decodeJWT(token) {
  try {
    return {
      payload: jwt.decode(token, this.jwt.secret)
    };
  }
  catch(err) {
    return {
...
function sendToMicroService(socketClient, data, application, handleResponse) {
  var q = this;
  debug('socketClient sending: %s', JSON.stringify(data));
      ...
      if (responseObj.message.token) {
        ...
        if (payload.application !== application) {
          ...
          encodeJWTInWorker.call(q, payload, function(jwtObj) {
            debug('encodeJWTInWorker - jwtObj = %s', JSON.stringify(jwtObj));
...
function masterRequest(data, socket, handleResponse) {
  ...
  if (this.jwt && this.jwt.secret) {
    ...
    decodeJWTInWorker.call(this, data.token, function(responseObj) {
      ...
      debug('decodeJWTInWorker - responseObj = %s', JSON.stringify(responseObj));
      ...
      if (q.u_services && q.u_services.byApplication[application] && q.u_services.byApplication[application][data.type]) {
        ...
        debug('incoming request for %s: %s will be handled by microservice', application, data.type);
...
function validate(messageObj) {
  ...
  try {
    payload = jwt.decode(messageObj.token, this.jwt.secret);
  }
  catch(err) {
    return {
      error: 'Invalid JWT: ' + err,
...
function getProperty(propertyName, token) {
  ...
  if (typeof payload[propertyName] === 'undefined') return false;
