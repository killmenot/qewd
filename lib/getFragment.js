...
var fs = require('fs');
var debug = require('debug')('qewd:getFragments');
...
module.exports = function(messageObj, application, finished) {
  ...
  debug('message: %s', JSON.stringify(messageObj));
  debug('application: %s', application);
  ...
  debug('fragment path: %s', fragPath);

  var exists = fs.existsSync(fragPath);
  if (!exists) {
    var message = {
  ...
  fs.readFile(fragPath, 'utf8', function(err, data) {
    if (err) {
      return finished({
        error: 'Unable to read file ' + fragPath,
        reason: err
      });
    }
