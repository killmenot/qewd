...
var debug = require('debug')('qewd:appHandler');
...
function loadModule(application, finished) {
  try {
    ...
    debug('handler module loaded for application ' + application + '; module: ' + moduleName);
    ...
    debug('error: %s, reason: %s', errorResponse.error, errorResponse.reason);
  ...
  this.on('message', function(messageObj, sendFn, finishedFn) {
    ...
    if (type === 'ewd-qoper8-express') {
      ...
      if (messageObj.headers && messageObj.headers.qewd && messageObj.headers.qewd === 'ajax') {
        ...
        send = function(msg) {
          debug('unable to use send() function over Ajax for intermediate message: %s', JSON.stringify(msg));
        };
      }
      ...
      debug('ewd-qoper8-express message remapped to type: %s, message: %s', type, JSON.stringify(messageObj));
    }
    ...
    if (type === 'ewd-reregister') {
      ...
      debug('reregister token: %s', messageObj.token);
      if (messageObj.jwt) {
    ...
    if (this.userDefined.config.lockSession) {
      ...
      debug('session locked: %s', JSON.stringify(this.sessionLocked));
    ...
    if (service) {
      ...
      debug('servicesAllowed: %s', JSON.stringify(servicesAllowed));
      ...
      debug('sessionAllowService: %s', sessionAllowService);
      debug('allowService: %s', allowService);
    ...
    if (this.handlers[application][type]) {
      finalise = function(results) {
        debug('finalise: results: %s', JSON.stringify(results));
        debug('finalise: session: %s', JSON.stringify(session));
        results = results || {};
